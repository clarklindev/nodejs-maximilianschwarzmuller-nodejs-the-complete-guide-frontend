    ...
    <div className={styles.products}>
      ...
      {((queryPage && queryItems) || page) && (
        <div className="paginationButtons">
          ...
          {page < totalPages && <button onClick={() => updatePage('next')}>Next</button>}
        </div>
      )}

      {products.map((product, index) => {
        return (
          <NavLink className={styles['nav-link']} to={`${product._id}`} key={index}>
            <p>{product.title}</p>
            <img
              src={`${import.meta.env.VITE_BACKEND_URL}:${import.meta.env.VITE_BACKEND_PORT}/images/${
                product.imageUrl
              }`}
              width="200"
              height="auto"
...
export const loader = async ({ request }) => {
  ...
  const domain = `${import.meta.env.VITE_BACKEND_URL}:${import.meta.env.VITE_BACKEND_PORT}`;
